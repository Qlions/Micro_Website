		...
		<div class="appoint-time-content" v-if="dataHide == 0">
			...
			<ul class="date-content">
				...
				<li v-for="(list,index) in timeList" :class="{'active' : list.count > nowCount}" @click="chooseTime(list.time,list.count,list.isFull)">
					<p v-if="list.isFull">{{ judegTime(list.time,timeJudegs) }}</p>
					<p>{{ list.time }}</p>
				</li>
			</ul>
		</div>
	...
	export default {
		...
		methods:{
			initData(){
				...
				this.$http.post('/webserviceProxy/webservice/wtWebApiH/GetServerData',{token:token, infoType:111189, jsonValue:data},{emulateJSON:true}).then( response => {
					...
					if(this.dateList.length > 0){
						...
						let timeDate = this.dateList[0].timeScope;
						if(this.dateList[0].timeScopeOnsite != ""){
							timeDate += (","+this.dateList[0].timeScopeOnsite)
						}
						//全部时间
						let time =this.uniqueArr(timeDate.split(','));
						let timeCan = this.dateList[0].timeScopeCan.timeScopeOk.split(',');
						let timeJudeg = "";
						if(this.dateList[0].timeScopeCan.timeScopeOk != ""){
							timeJudeg += this.dateList[0].timeScopeCan.timeScopeOk;
						}
						if(this.dateList[0].timeScopeCan.timeScopeOut != "" && this.dateList[0].timeScopeCan.timeScopeOk == ""){
							timeJudeg += this.dateList[0].timeScopeCan.timeScopeOut;
						}
						if(this.dateList[0].timeScopeCan.timeScopeOut != "" && this.dateList[0].timeScopeCan.timeScopeOk != ""){
							timeJudeg += (","+this.dateList[0].timeScopeCan.timeScopeOut);
						}
						vm.timeJudegs = timeJudeg.split(",")
						console.log(vm.timeJudegs);
			...
			tabDate(index,dataDay){//日期切换
				...
				this.isActive = index
				//获取时间列表
				let timeDate = this.dateList[index].timeScope;
				if(this.dateList[0].timeScopeOnsite != ""){
					timeDate += (","+this.dateList[index].timeScopeOnsite)
				}
				let timeJudeg = "";
				if(this.dateList[index].timeScopeCan.timeScopeOk != ""){
					timeJudeg += this.dateList[index].timeScopeCan.timeScopeOk;
				}
				if(this.dateList[index].timeScopeCan.timeScopeOut != "" && this.dateList[index].timeScopeCan.timeScopeOk == ""){
					timeJudeg += this.dateList[index].timeScopeCan.timeScopeOut;
				}
				if(this.dateList[index].timeScopeCan.timeScopeOut != "" && this.dateList[index].timeScopeCan.timeScopeOk != ""){
					timeJudeg += (","+this.dateList[index].timeScopeCan.timeScopeOut);
				}
				this.timeJudegs = timeJudeg.split(",");
				console.log(this.timeJudegs);
				//预约时间
