...
<script>
	...
	export default {
		...
		methods:{
			...
			chooseTip(item,index){//选择某个筛选条件
				this.$store.state.home_item = ""; // 清空首页跳转参数
				this.$store.state.home_index = "";// 清空首页跳转参数
				let vm = this;
			...
			chooseTipHome(item,index){ //首页跳转函数
				let vm = this;
				let userInfo =  JSON.parse(this.$cookie.get('userInfo'));
				let token;
				if(!userInfo){
				...
				vm.$http.post('/webserviceProxy/webservice/wtWebApiH/GetServerData',{token:token, infoType:111180, jsonValue:JSON.stringify(vm.filterDatas)},{emulateJSON:true}).then( response => {
					console.log(response);
					vm.addedHealthList = response.body.rows;
					vm.signState = response.body.signState;
					if(vm.addedHealthList.length < 4){
						document.getElementsByClassName("_v-content")[0].style.height = 870 + 'px';
						vm.$refs.my_scroller.scrollTo(0,96,true);
					}else {
			...
			initHealthList(){//初始化服务列表
				...
				//服务列表接口
				if ( this.$store.state.home_item != "" ){
					this.chooseTipHome(this.$store.state.home_item,this.$store.state.home_index);
				}else {
			...
			getPosition(){//本地地理位置定位，获取本地经纬度并存到本地
				let vm = this;
				if(vm.$store.state.home_item == ""){
			...
			tabView(index,item){//筛选栏点击
				this.$store.state.home_item = "";
				this.$store.state.home_index = "";
				let vm = this;
				let _top = vm.$refs.my_scroller.getPosition().top;
				if(_top < 93){
			...
			addressPoint(){//点击事件，进入地址选择页面
				let vm = this;
//				vm.$router.push({name: 'shareVideo',query: {liveStreamingId: '64146201807717376100001'}});
//				return false;
			...
			listenToMyBoy(somedata){//监听头部header
