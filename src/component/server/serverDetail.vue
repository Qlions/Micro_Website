	...
	<div id="serverDetail">
		...
		<div class="serDetail_content">
			...
			<div class="server_info">
				...
				<p class="server-source" v-if="!isShow"  @click="agDeta"><img src="../../assets/image/server/serviceEa.png" alt=""/>由 <span>{{ detailObj.eaName }}</span> 提供服务</p>
				<div @click="servicePlace" class="server-address" v-if="isAddress">
					<img src="../../assets/image/server/server-address.png" class="img-icon" />
					{{ addressName }}
					
					<img src="../../assets/image/server/server-right.png" class="server-right"/>
				</div>
		...
		<div class="server-pay clearfix">
			<!--<div class="server-price server-pay-area" v-if="isEqual">¥<span>{{ Number(serviceMoney) | LegalNumber}}</span><i v-if="isHaveItems">+</i></div>-->
			<div class="server-price server-pay-area"><span v-html="serverMoney(detailObj)"></span></div>
		...
		methods: {
			initDetail() {//初始化服务详情所需数据
				...
				this.$http.post('/webserviceProxy/webservice/wtWebApiH/GetServerData', {token: token,infoType: 111181,jsonValue: data}, {emulateJSON: true}).then(response => {
					...
					//获取医生ID
					this.serviceDictId = response.body[0].serviceDictId;
					let _name = this.detailObj.serviceDictName;
					let _imgUrl = this.imgIcon(this.detailObj.imgUrl);
					this.wxshare(_name,_imgUrl);
					setTimeout(() => {
						this.isLoading = false;
					}, 400)
					if(this.detailObj.isHaveItems.length > 0){
					...
					if(this.imgList.length > 1) {//判断图片预览是否是多张
						this.isMore = true;
						this.imgCount = this.imgList.length;
					} else {
						this.isMore = false;
					}
					if(this.detailObj.serviceDictWay == "门诊(店)服务" || this.detailObj.serviceDictWay == "综合服务") {//如果当前服务类型是门诊店服务
						this.isAddress = true;
						let vm = this;
						...
						this.$http.post('/webserviceProxy/webservice/wtWebApiH/GetServerData', {token: token,infoType: 111177,jsonValue: addressData}, {emulateJSON: true}).then(response => {
							console.log(response);
							this.eaId = response.body.rows[0].eaId;
							if(eaId == null){
								window.sessionStorage.setItem("server_eaId",response.body.rows[0].eaId)
							}
...
//					console.log(this.detailObj);
			...
			serverMoney(obj){//服务价格filter
				let min = Number(obj.serviceMoneyMin);
				let max = obj.serviceMoneyMax;
				let isHaveItems= obj.isHaveItems;
				if(min == max){
					if(isHaveItems == '1'){
						return "<i>¥</i>" + min.toFixed(2) + "<em class='emm'>+</em>";
					}else {
			...
			signEvent(data) {//监听是否需要签约
				this.isSign = data;
			},
			waitEvent(data){//监听是否等待审核
				console.log("休息休息",data);
				this.isWait = data;
			},
			teamEvent(data){//监听是否当前团队是否开通服务
				this.isTeam = data;
			},
			backToIndex() {//返回首页
				if(this.$route.query.docuserId) {//如果是从专家服务列表进入
			...
			goToConfirm() {//进入服务中间页
				...
				if (!userInfo) {
					...
					if(isSign.includes('yyyz6')){//需要签约家庭医生
						if(signState == '0') {//未签约
							this.isSign = true;
							return false;
						}
			...
			setIframeHeight(iframe) {
				...
				let h = document.getElementById('external-frame').contentWindow.document.documentElement.offsetHeight;
				iframe.height = h;
			...
			wxshare(n,u){
				...
				let _title = n;
				let _linkUr2 = window.location.href;
				let _imgUrl2 = u;
				//获取nickname
//				let _wxMsg = this.$cookie.get("wxMsg");
				let _desc = '我在中国健康云发现了一个不错的服务，赶快来看看吧，一起共享健康！';
				// 获取签名等参数
			...
			//						    openMessageAlert("当前客户端不支持指定JS接口","2");
							...
							wx.onMenuShareTimeline({
							    title: _title, // 分享标题
							    link: _linkUr2, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
							    imgUrl: _imgUrl2, // 分享图标
							    success: function () { 
							        // 用户确认分享后执行的回调函数
							    },
