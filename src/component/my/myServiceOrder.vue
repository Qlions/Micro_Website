...
<section id="orderList">
		...
		<div class="li_box" v-for="(item,index) in order">
			<div class="O-content" >
				...
				<div class="order-bottom post-talk" v-if="item.orderState == 2"><!--已完成底部样式-->
					...
					<router-link class="buy-again-zixun" :to='({path:"/my/feedback",query:{ type : 4,typeId:item.serviceDictId,title:"问题咨询" }})'>选此咨询</router-link>
				</div>	
	...
	<div class="footer_btn">
		<ul>
			<li class="problem"><router-link :to='({ path:"/my/myServiceOrder" })'>订单问题</router-link></li>
			<li class="feedbacks"><router-link :to='({ path:"/my/feedback",query:{ type : 10,title:"问题反馈" } })'>问题反馈</router-link></li>
			<li class="complaint"><router-link :to='({ path:"/my/feedback",query:{ type : 12,title:"投诉建议" } })'>投诉建议</router-link></li>
		</ul>
	</div>
</section>
</template>
...
<style scoped>	
	...
	.problem a {
		color: #65b351 !important;
	}
	.problem {
		background: url(../../assets/image/my/icon_service_min.png) no-repeat center left .5rem;
		background-size: .35rem .38rem;
	}
	.feedbacks {
		background: url(../../assets/image/my/icon_service_min2.png) no-repeat center left .5rem;
		background-size: .35rem .38rem;
	}
	.complaint {
		background: url(../../assets/image/my/icon_service_min3.png) no-repeat center left .5rem;
		background-size: .35rem .38rem;
	}
</style>