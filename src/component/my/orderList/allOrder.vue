			...
			<div class="O-content" >
				<div class="order-title">
					<router-link :to="'/my/orderCode/' + item.orderId +'/'+item.orderCode + '/' +item.orderState" class="codeDetail">	
						...
						<p class="order-title-type" v-if="item.orderState == 2">
							已完成
						</p>
						<p class="order-title-type" v-if="item.orderState == 6">
							已取消
						</p>
						<p class="order-title-type" v-if="item.orderState == 7">
							待确认
						</p>
						<p class="order-title-type" v-if="item.orderState == 0&&timeOverState">
							已取消
						</p>
						<p class="order-title-type" v-if="item.orderState == 8">
							处理中
						</p>
					</router-link>
				</div>
				<div class="order-content">
					<router-link :to="'/my/orderDetail/'+item.orderId" class="content1" @click="goOrderDetail(item.orderId)">
				...
				<div v-if="item.orderState == 4&&item.serviceComponent !='图文服务'&&item.startTime !=''" style="height: 0.2rem; width: 100%;">
				...
				<div class="order-bottom" v-if="item.orderState == 0&&timeOver" style="width: 95%;margin: 0 auto;">
					<i v-if="timeOver" style="font-size:0.24rem;display: inline-block;height: 0.8rem; line-height: 0.8rem;margin-top: -0.1rem;color: #ff5454;" :id="'timer'+index">
						<!-- {{ countDown(item.remainderTime,'timer'+index) }} -->
					 </i>
					<p v-if="timeOver" @click="goPay(item.orderId)" style="background: #ff5454;border: 1px solid #ff5454;color: #fff;margin-right: 0;">支付</p>
					<p v-if="timeOver" @click="cancle(item.orderId,item.eaId,item.serviceReservation,item.reservationMode,item.paymentChannel)">取消服务</p>
				</div>
				<div class="order-bottom" v-show="item.orderState == 4&&item.startTime ==''&&item.serviceComponent !='图文服务'">
					<p @click="cancle(item.orderId,item.eaId,item.serviceReservation,item.paymentChannel)">取消服务</p>
				</div>
				...
				<div class="order-bottom post-talk" v-if="item.orderState == 2"><!--已完成底部样式-->
					...
					<p class="buy-again" @click="open(item.orderId)">删除</p>
					<router-link class="buy-again" :to='({path:"myServerDetail",query:{id:item.serviceDictId,type:(item.serviceReservation==true?"1":"0"),docuserId:item.docId,serviceId:(item.serviceId == null ? item.serviceDictId : item.serviceId),money:Number(item.payMoney).toFixed(2)}})'>再次购买</router-link>
				</div>	
...
//		    },
			...
			serverList () {
				...
				this.$http.post('/webserviceProxy/webservice/wtWebApiH/GetServerData',{token:token, infoType:660086, jsonValue:data},{emulateJSON:true}).then( response => {
					...
					for(let i in this.order){
						...
						this.time = Number(timeBox[i]+60);
						if(this.order[i].orderState == 0){
							this.countDown( Number(timeBox[i]+60),'timer'+i);
						}
					}
			...
			countDown (maxtimes,id) { 
				var that = this;
				var maxtime = maxtimes;
				var name = id;	
				//
				if(maxtime>0){
					...
					that.timer = setInterval(function() {  
				        if(maxtime>=0){	
				            ...
				            if((minutes == '00')||minutes == '0-1'){
				             	document.getElementById(name).innerHTML = tip;
				            }
...
//					        	console.log('可以执行吗？');
			...
			//取消按钮点击事件
			cancle(orderId,eaId,serviceReservation,reservationMode,paymentChannel){
				console.log(serviceReservation);
			...
			cancleOrder(){
				...
				if(that.remark !=''){
					...
					if(that.paymentChannel == 0){//通过微信或支付宝付款
						that.$http.post('/webserviceProxy/webservice/wtWebApiH/GetServerData',{token:token, infoType:660120, jsonValue:str},{emulateJSON:true}).then( response => {
							...
							console.log(that.paymentChannel);
			...
			goPay(orderId){//支付
				this.$router.push({"path":"/serverConfirm/serverOrder/orderPayment",query:{orderId:orderId}})
			},
			buyAgain(){
