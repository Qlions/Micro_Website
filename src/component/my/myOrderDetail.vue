	...
	<div class="OD-container">
		...
		<div class="order-content white">
			<dl @click="goServerDetail(order.serviceDictId,order.serviceReservation)">
				...
				<dd>
					...
					<div class="dd-p dd-p3">
						...
						<p class="dd-p3-right"><span>￥{{order.servicePayMoney?(order.servicePayMoney.toFixed(2)):"0.00"}}</span></p>
					</div>
				</dd>
			</dl>
			...
			<div class="doc-content">
				...
				<p class="serverdizhi" v-if="order.serviceWay =='门诊(店)服务'" @click="goMap">
		...
		<div class="payType white">
			<p>
				<span>支付方式</span><em>{{order.inOutType==0?"在线支付":""}}</em>
			</p>
		</div>
		<div class="payMoney">              
            <p class="order_ly">订单<span class="orderMoney_ly">￥<span>{{order.servicePayMoney?order.servicePayMoney.toFixed(2):"0.00" }}</span></span>
            </p>
            <p class="truePay_ly">实付：<span class="trueMoney_ly">￥<span>{{order.payMoney?order.payMoney.toFixed(2):"0.00"}}</span></span></p>
		...
		<div class="pinlvBox" v-show="order.orderState == 3" style="margin-top: 0.15rem;">
			<span class="quxiao" @click="cancle(order.orderId,order.eaId,order.paymentChannel)">取消服务</span>
		</div>
		...
		<div class="order-bottom" v-if="order.orderState == 0&&timeOver" style="width: 100%;margin: 0 auto;">
			...
			<p v-if="timeOver" @click="cancle(order.orderId,order.eaId,order.paymentChannel)">取消服务</p>
		</div>
		
		<div class="pinlvBox zixunBox" v-show="order.orderState == 2&&order.startLevel!=''">
			<router-link class="fin-buy-again" style="position: relative; z-index: 1;top: -0.3rem;right: 1.4rem;" :to='({path:"/my/myServerDetail",query:{id:order.serviceDictId,type:(order.serviceReservation==true?"1":"0"),docuserId:order.serviceUserId,serviceId:order.serviceId,money:Number(order.payMoney).toFixed(2)}})'>再次购买</router-link>
...
<script>
	...
	export default {
		...
		mounted: function() {
			if(sessionStorage.getItem('key_head') == 1){//判断头部是否下载
...
//				this.$router.push({ name: 'consultDetail', params: { id: order.QId,order:order}})
			...
			//取消按钮点击事件
			cancle(orderId,eaId,paymentChannel){
				this.isCancle = true;
				this.orderId = orderId;
				this.eaId = eaId;
