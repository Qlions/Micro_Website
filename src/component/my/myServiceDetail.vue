<template>
	<section id="consultDetail">
		...
		<div class="consultD-container">
			...
			:style="{ top:.84+'rem' }"
			>
				<p v-if="assessData.QState == 2" class="countTime">{{ hours }}小时{{ minutes }}分后系统自动关闭咨询</p>
				<ul class="consultD-list">
					<li class="consultD-list-item" v-for="(count,index) in arrList">
						...
						<div v-if="count.source==0"><!--问答模块-->
							...
							<!--医生回复模块-->
							<div class="other-message other-message-docrot" v-if="count.AId != null &&count.AId!=''">
								<div class="null" v-if="count.AcreateTime">
									...
									<div class="list-right">
										<div class="list-right-box">
											<div class="list-right-t">
												<span class="consultTime" v-if="count.AcreateTime" style="float: left;">{{ count.AcreateTime?count.AcreateTime.substring(0,10):""}}
			...
			<div class="foot" id="footTalk" v-if="assessData.QState != 4">
				<div class="service_evaluate" v-if="evaluatePoVal == 1" @click="Getvaluate">
					<span>如果问题已解决请评价</span>
				</div>
				<div class="messSend" v-if="assessData.QState != 5">
			...
			<div class="foot_evaluate" v-if="assessData.QState == 4" @click="completePo">
				<span>服务评价</span>
			</div>
		</div>
		...
		<div class="evaluate_popup" v-if="SeeEvaluatePoPage">
			...
			<div class="see_evaluate_popup_content">
				...
				<div class="see_evaluate_popup_content_text">
					...
					<div class="content-editable">{{ assessData.commentContent }}</div>
				</div>
			</div>
		</div>
	</section>
		...
		methods: {
			...
			consultDetail:function(){
				...
				this.$http.post('/webserviceProxy/webservice/wtWebApiH/GetServerData',{token:token, infoType:660132, jsonValue:data},{emulateJSON:true}).then( response => {
					...
					let vm = this;
					let total = response.body.map.minDate;
					this.minDate = response.body.map.minDate;
					this.countTime(total);
...
//					let focus = that.$refs.footTalk
			...
			countTime (total) {
				...
				let int = setInterval(function(){
					vm.hours = total/3600 < 10 ? ('0' + parseInt(total/3600)) : parseInt(total/3600);
					vm.minutes = (total-vm.hours*3600)/60 < 10 ? ('0' + parseInt((total-vm.hours*3600)/60)) : parseInt((total-vm.hours*3600)/60);
					total--;
				}, 1000);
				if(total < -1) {
					clearInterval(int);
...
<style>
	...
	.see_evaluate_popup_content .content-editable {
		width: 100%;
		padding: .3rem 0;
		border: none;
	    resize: none;
