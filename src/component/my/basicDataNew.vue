<template>
	<section class="basicData">
		<Heads :title="titleName">
			<div class="postUserInfo" @click="postUserInfo">保存</div>
		</Heads>
		...
		<div class="BD-container clearfix">
			...
			<div class="basicData1 clearfix">
				<ul class="basicData-content1 basicData-content clearfix">	
					...
					<li>
						<label for="n_birth">
							<span><b class="red">*</b>出生日期：</span>
							<input type="text" placeholder="输入日期" name="appDate" id="appDate"  @click="birthdayList" v-model="userInfo.birthDate">
						</label>
					</li>
					...
					<li>
						<label>
							<span>所在省份：</span>
							<input name="province" readonly="readonly" @click="provinceList()" type="text" placeholder="选择所在省份" v-model="userInfo.provinceName"> 
						</label>
					</li>
					...
					<li>
						<label>
							<span>所在城市：</span>
							<input name="city" readonly="readonly" @click="cityList()" type="text" placeholder="选择所在城市" v-model="userInfo.cityName">
						</label>
					</li>
					...
					<li>
						<label>
							<span>所在区县：</span>
							<input name="area" readonly="readonly" @click="areaList()" type="text" placeholder="选择所在区县" v-model="userInfo.areaName">
						</label>
					</li>
				...
				<ul class="basicData-content2 basicData-content">
					...
					<li>
						<router-link to="/my/checkTel" class="dataList">
							<label>
								<span><b class="red">*</b>电话号码：</span>
								<input type="text" placeholder="请输入手机号码" v-model="userInfo.mobile" disabled="disabled">
							</label>
						</router-link>
					</li>
					...
					<li>
						...
						<label>
							<span>工作单位：</span>
							<input type="text" placeholder="请输入工作单位" v-model="userInfo.ROrgName">
						</label>
					</li>
					...
					<li>
						<label>
							<span>通信地址：</span>
							<input type="text" placeholder="请输入通信地址" v-model="userInfo.address">
						</label>
					</li>
					...
					<li>
						<label>
							<span>邮编：</span>
							<input type="text" placeholder="请输入邮编" v-model="userInfo.postCode">
						</label>
					</li>
			...
			<div v-show='isShow_pro'style="width: 6rem;" >
				...
				<div class="my-province-container" style=" width: 80%;height: 50%;background: #fff;position: fixed;top: 20%;left: 10%;z-index:999;border-radius: 3px;overflow:auto;">
...
//						that.userInfo.areaName = that.response.areaCode;
		    ...
		    postUserInfo:function(){
		    	...
		    	if( that.userInfo.name == '' ){
		    		that.errMask("请输入您的真实姓名");
		    	} else if( that.userInfo.username == '' ){
		    		that.errMask("请输入您的昵称");
		    	} else if( that.userInfo.sex == '' ){
		    		that.errMask("请选择您的性别");	
		    	} else if( that.userInfo.birthDate == '' ){
		    		that.errMask("请选择您的出生日期");		
		    	} else if( that.userInfo.mobile == '' ){
		    		that.errMask("请输入您的手机号码");	
		    	} else if(that.userInfo.username&&userInfo.userName == userInfo.userCode){
