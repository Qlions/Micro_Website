<template>
	<div class="healthy-medication-add-details">
		<Heads :title="title">
			<span class="remove_btn" @click="removeList">
				删除
			</span>
		</Heads>
					...
					<ul>
						<li>
							<div class="details-drugs-data-txt">
								<input placeholder="输入药品名称" v-model="item.medicineName_1">
							</div>
						</li>
						<li>
							<div class="medication-drugs-title">用药日期</div>
							<div class="details-drugs-date-num">
								<input placeholder="选择用药日期" readonly="readonly" name="appDateTime" :id="'appDateTime'+index" type="text" v-model="item.medicineTime_1" onfocus="this.blur()">
							</div>
						</li>
						<li>
							...
							<div class="details-data-drugs-usage">
								<input type="text" @click="togglePop(index)" readonly="readonly" :placeholder="domList[index].usageType_1" v-model="domList[index].usageType_1" onfocus="this.blur()">
						...
						<li>
									...
									<div class="details-data-drugs-usage-pop-content">
										...
										<ol class="drugUsage">
											<li v-for="(item,index) in consumption">
												<label :for="(item,index).name" @click="isname3(index)" :class="{'on':item.name == isName3 }">
							...
							<div class="details-data-drugs-date">
								<span @click="togglePop_frequency(index)">
									<input type="text" readonly="readonly" placeholder="用药次数" v-model="domList[index].timesDay_1" class="drugs-num" onfocus="this.blur()">
									次
								</span>
						...
						<li>
							...
							<div class="details-data-drugs-usage down">
								...
								<span>
									<input type="number" @focus="focus" @blur="blur"  placeholder="每次剂量" v-model="item.dosage_1" class="every">
								</span>
		...
		mounted () {
			...
			for (var i = 0; i < data.length; i++) {
				...
				if( this.name == data[i].medicineName ){
					...
					let medicineTime_1 = this.details.medicineTime.replace('T',' ');
					this.domList[0].medicineTime_1 = medicineTime_1.substring(0,16);
...
//					let focus = that.$refs.footTalk
			...
			post_usage () {
				...
				if(this.drugUsageNameNew == '外用' || this.drugUsageNameNew == '静脉点滴'){
					...
					this.domList[this.isIndex].usageTime_1 = '用药时间';
				}
				this.isIndex = -1;
			},
			// 用药时间-》》方法
			...
			submit () {
				this.domListNew = this.domList;
				let userInfo =  JSON.parse(this.$cookie.get('userInfo'));
				let token = userInfo.token;
				let newdomList = this.domList;
				let postData = {};
				for (var i = 0; i < this.domListNew.length; i++) {
					for (var p1 in this.domListNew[i]) {
				        var tr = p1.split("_");
				       	if(newdomList[i][p1] == ''){
				        	this.show = true;
...
<style>
	...
	.details-data-drugs-date span,
	.details-data-drugs-usage span {
		text-align: right;
		display: block;
	}
	.drugs-num {
