<template>
	<div class="healthy-medication-add-details">
					...
					<ul>
						...
						<li>
							...
							<div class="details-drugs-date-num">
								<input placeholder="选择用药日期" class="" readonly="readonly" name="appDateTime" :id="'appDateTime'+index" type="text" value="" onfocus="this.blur()">
							</div>
						</li>
						<li>
		...
		<div class="preservation" id="footer">
			<a href="javascript:void(0)" @click.prevent="submit">
				保存
			</a>
		</div>
		<!--点击退出时的弹出框-->
	...
	export default {
		...
		methods:{
			...
			submit () {
				...
				const newdomList = this.domList;
				let postData = {};
				console.log(this.consumption)
				for (var i = 0; i < newdomList.length; i++) {
					for (var p1 in newdomList[i]) {
				        var tr = p1.split("_");
				        if(newdomList[i][p1] == ''){
				        	this.show = true;
				...
				postData['divindex'] = newdomList.length;
