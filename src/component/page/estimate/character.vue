						...
						<ul>
							<li v-for="item in lists">
								<div v-for="(data,val,index) in item">
									...
									<div v-for="(character,val,index) in data">
										<div v-if="character.flag == 0" @click="temporary()">
											<h2 class="date">{{ character.createTime | substring(5,10) }}</h2>
											<p class="up flagtp">
												<i></i>
												<font>{{ character.createTime | substring(11,16) }}</font>
												<b>暂存</b>
												<span>一次{{ albumName }}</span>
											</p>
										</div>
										<div v-if="character.flag == 1" @click="complete(character.pressureId)">
											<h2 class="date">{{ character.upTime | substring(5,10) }}</h2>
											<p class="up flagcp">
												<i></i>
												<font>{{ character.upTime | substring(11,16) }}</font>
												<b>完成</b>
												<span>一次{{ albumName }}</span>
											</p>
										</div>
...
<script>
	...
	export default {
		...
		computed:{
		    ...
		    albumUrl () {
		    	var mapAlbum = {
					character: ""+this.questionnairegUrl+"/newReport/xg/index_xg1.html?pressureId=&gender="+this.gender+"&userId="+this.userId+"&token="+this.token+""
				}
				return mapAlbum[this.title]
		    }    
		},
		methods:{
			...
			build (){
				...
				if( title == 'character' ){
					...
					.then(response => {
						if( response.body.flag == 2 && response.body.typeData == 5){
			...
			// 问卷的路径
			temporary () {
				window.location.href = ""+this.questionnairegUrl+"/newReport/xg/index_xg1.html?pressureId=&gender="+this.gender+"&userId="+this.userId+"&token="+this.token+"";
			},
			// 报告的路径
			complete (pressureId) {
				window.location.href = ""+this.configUrl+'/website/mobile/pressureEvaluteReportShow.action?typeData='+5+'&'+'pressureId='+pressureId+'&'+'flagInfo='+1+"";
			},
			clearStorage () { //清楚定时器
				let time = setTimeout(()=>{
					if( this.showUp ==  true){
