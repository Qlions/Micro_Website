			...
			<div class="scroller-content" v-if="dataHide == 1">
		    	...
		    	>
					<div class="home-content-value">
						<ul>
							<li v-for="item in lists" v-if="title == 'pressure'">
								<div v-for="(data,val,index) in item">
									<h2 class="year-line" v-if="val != year">
										<span>{{ val | substring(0,4) }}</span>
									</h2>
									<div v-for="(pressure,val,index) in data">
										<div v-if="pressure.flag == 0" @click="temporary()">
											<h2 class="date">{{ pressure.createTime | substring(5,10) }}</h2>
											<p class="up flagtp">
												<i></i>
												<font>{{ pressure.createTime | substring(11,16) }}</font>
												<b>暂存</b>
												<span>一次{{ albumName }}</span>
											</p>
										</div>
										<div :href="[configUrl+'/website/mobile/pressureEvaluteReportShow.action?'+'typeData='+6+'&'+'pressureId='+pressure.pressureId+'&'+'flagInfo='+1]" v-if="pressure.flag == 1" @click="complete(pressure.pressureId)">
											<h2 class="date">{{ pressure.upTime | substring(5,10) }}</h2>
											<p class="up flagcp">
												<i></i>
												<font>{{ pressure.upTime | substring(11,16) }}</font>
												<b>完成</b>
												<span>一次{{ albumName }}</span>
											</p>
										</div>
...
<script>
	...
	export default {
		...
		computed:{
		    ...
		    albumUrl () {
		    	var mapAlbum = {
					pressure: ""+this.questionnairegUrl+"/newReport/yl/index_yl1.html?pressureId=&gender="+this.gender+"&userId="+this.userId+"&token="+this.token+""
				}
				return mapAlbum[this.title]
		    }    
		},
		methods:{
						...
						setTimeout(() => {
							...
							if(  vm.list == "" || vm.list == undefined ){
								...
								for (var i = 0; i < vm.list.length; i++) {
									if( vm.list[i].flag == 1 ){
										let moth = vm.list[i].upTime.substring(0,5);
										mothArr.push(moth);	
									}else {
			...
			build (){
				...
				if ( title == 'pressure' ) {
					...
					.then(response => {
						console.log(response)
						if( response.body.flag == 2 && response.body.typeData == 6 ){
							let upTime = response.body.upTime;
							this.startDate = upTime.substring(0,upTime.length-2);
			...
			// 问卷的路径
			temporary () {
				window.location.href = ""+this.questionnairegUrl+"/newReport/yl/index_yl1.html?pressureId=&gender="+this.gender+"&userId="+this.userId+"&token="+this.token+"";
			},
			// 报告的路径
			complete (pressureId) {
				window.location.href = ""+this.configUrl+'/website/mobile/pressureEvaluteReportShow.action?typeData='+6+'&'+'pressureId='+pressureId+'&'+'flagInfo='+1+"";
			},
			clearStorage () { //清楚定时器
				let time = setTimeout(()=>{
					if( this.showUp ==  true){
