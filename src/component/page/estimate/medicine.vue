						...
						<ul>
							<li v-for="item in lists">
								<div v-for="(data,val,index) in item">
									...
									<div v-for="(medicine,val,index) in data">
										<div v-if="medicine.flag == 0" @click="temporary()">
											<h2 class="date">{{ medicine.createTime | substring(5,10) }}</h2>
											<p class="up flagtp">
												<i></i>
												<font>{{ medicine.createTime | substring(11,16) }}</font>
												<b>暂存</b>
												<span>一次{{ albumName }}</span>
											</p>
										</div>
										<div v-if="medicine.flag == 1" @click="complete(medicine.tcmId)">
											<h2 class="date">{{ medicine.upTime | substring(5,10) }}</h2>
											<p class="up flagcp">
												<i></i>
												<font>{{ medicine.upTime | substring(11,16) }}</font>
												<b>完成</b>
												<span>一次{{ albumName }}</span>
											</p>
										</div>
...
<script>
	...
	export default {
		...
		computed:{
		    ...
		    albumUrl () {
		    	var mapAlbum = {
					medicine: ""+this.questionnairegUrl+"/newReport/zy/index_zy1.html?tcmId=&gender="+this.gender+"&userId="+this.userId+"&token="+this.token+""
				}
				return mapAlbum[this.title]
		    }    
		},
		methods:{
				...
				if( title == 'medicine' ){
					...
					.then(response => {
						if( response.body.flag == 1 ){
							this.startDate = response.body.upTime;
			...
			// 问卷的路径
			temporary () {
				window.location.href = ""+this.questionnairegUrl+"/newReport/zy/index_zy1.html?tcmId=&gender="+this.gender+"&userId="+this.userId+"&token="+this.token+"";
			},
			// 报告的路径
			complete (tcmId) {
				window.location.href = ""+this.configUrl+'/website/mobile/tcmReport.action?tcmId='+tcmId+'&'+'forward='+'summaryAll'+'&'+'flagInfo='+1+"";
			},
			clearStorage () { //清楚定时器
				let time = setTimeout(()=>{
					if( this.showUp ==  true){
