										...
										<div class="key" v-for="(listsNxt,key,index) in content">
											<router-link :key="listsNxt.infoId" :to="{ name:assessmentTitle,query:{
												...
												<div class="up">
													<i></i>
													<p class="up_info">
														<font class="up_info_num">{{ judgeTime(listsNxt.measureTime) }}</font>
														<font>{{ listsNxt.bmiStr }}</font>
														<span>
															<em v-if="assessmentTitle == 'motion'">{{ listsNxt.sportKcal }}</em>
															<em v-if="assessmentTitle == 'drinkWine'">{{ split(listsNxt.valueNumber) }}</em>
															<em :class="{emcol:listsNxt.bmiStrVlaue ==3 ||listsNxt.bmiStrVlaue == 1 || listsNxt.bmiStrVlaue ==4}" v-if="assessmentTitle == 'weightIndex'">{{ split(listsNxt.valueNumber) }}</em>
															<em v-if="assessmentTitle == 'weight'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'theWaist'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'pulse'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'electrocardiogram'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'temperature'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'bloodPressure'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'bloodsugar'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'triglyceride'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'LDLC'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'HDLC'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'totalCholesterol'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'diet'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'smoke'">{{ listsNxt.valueNumber }}</em>
															<em v-if="assessmentTitle == 'spo2Info'">{{ listsNxt.valueNumber }}</em>
															<b v-if="assessmentTitle != 'weightIndex'">{{ listsNxt.dicUnit }}</b>
														</span>
													</p>
													<p v-if="assessmentTitle != 'motion'">
														<font>{{ listsNxt.description }}</font>
													</p>
													<p class="motion-name" v-if="assessmentId == 111612">
														<font>{{ listsNxt.sportName }}</font>
														<span v-if="assessmentTitle == 'motion'">
															<em v-if="listsNxt.readonly == 2">
																{{ formatSecondsFen(listsNxt.minuteSum) }}
															</em>
															<em v-else>{{ formatSeconds(listsNxt.minuteSum) }}</em>
														</span>
														<span v-else>{{ listsNxt.minuteSum }}</span>
													</p>
												</div>
											</router-link>
										</div>
				...
				.then(response => {	
					...
					setTimeout(() => {		
						...
						if( vm.data == "" || vm.data == undefined ){
							...
							this.dataHide = 1;
							for (var i = 0; i < vm.data.length; i++) {
								let moth = vm.data[i].measureTime.substring(5,10);
								mothArr.push(moth);	
							}
							newArr = Array.from(new Set(mothArr)); // 去重
							for(let j = 0; j < newArr.length; j ++){
