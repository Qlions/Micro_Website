			...
			<div class="diet-content">
				<ul>
					<li v-for="(item,index) in this.listArr">
						<p class="diet-name" @click="modality(item.sportName,item.minuteSum,item.kcalId,item.PsortId)">{{ item.sportName }}</p>
						<p class="diet-unit">{{ item.minuteSum }}<b>分</b><i class="diet-remove" @click="remove(index)"></i></p>	
					</li>
				</ul>
			</div>
		...
		<div class="diet-modality" v-show="isModality">
			<div class="diet-modality-form">
				...
				<div class="modality-btn">
					...
					<a href="javascript:void(0)" @click="commit(newValueTxt,sportName,kcalId,PsortId)">确定</a>
				</div>
			</div>
		</div>
	...
	export default {
		data(){
			return {
				...
				sportinfoId: this.$route.query.sportinfoId,
				newDateVal:"",
				newTimeVal:""
			}
		},
		...
		methods:{
			...
			preservation () { //提交记录
				...
				if( this.type == 2 ){
					...
					for (var i = 0; i < this.listArr.length; i++) {
						let val = this.listArr[i].minuteSum;
						let id = this.listArr[i].kcalId;
						strData += id+':'+val+';';
					}
					let data = JSON.stringify({
						'value':strData,
					...
					for (var i = 0; i < this.$store.state.listArr.length; i++) {
						let val = this.$store.state.listArr[i].minuteSum;
						let id = this.$store.state.listArr[i].kcalId;
						strData += id+':'+val+';';
					}
					let data = JSON.stringify({
						'value':strData,
			...
			addRecordBtn () {
				this.$router.push({ name: 'motionClass',query: { classA:111614,classB:111615  } })
			},
			remove (index) {
				this.$store.state.listArr.splice(index, 1);
				this.listArr.splice(index, 1);
			},
			modality (name,val,code,PsortId) {
				let vm = this;
				vm.isModality = true;
				vm.sportName = name;
				vm.kcalId = code;
			...
			commit (minuteSum,sportName,kcalId,PsortId) {
				let replacement = {
					minuteSum:minuteSum,
					kcalId:kcalId,
					sportName:sportName,				
				...
				for (let i = 0, len = this.$store.state.listArr.length; i < len; i++) {
				    if ( this.$store.state.listArr[i].sportName === replacement.sportName ) {
