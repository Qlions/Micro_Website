				...
				<ul class="input-box">
					<li>
						<label for="">
							<span>脉搏：</span>
							<div class="reg-int">
								<input type="number" placeholder="请输入脉搏值" v-model="valueTxt">
								<i>次/分</i>
							</div>
						</label>
					</li>
		...
		methods:{
			...
			inptUpdate (){
				if( this.type == 2 ){
					...
					this.valueTxt = this.valueNumber;	
			...
			initAjax (data) {
				...
				if( this.valueTxt < 50 ||this.valueTxt > 150 ){
					this.isLading = false;
					this.valueTxt = '';
					this.show = true;
					this.message = '数值范围应在50~150之间。';
					this.clearStorage();
				}else if( this.valueTxt%1 !== 0 ){
					if(this.valueTxt.toString().split(".")[1].length > 0){
						this.isLading = false;
						this.show = true;
