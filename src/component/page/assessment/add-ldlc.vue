				...
				<ul class="input-box">
					<li>
						<label for="">
							<span>LDL-C：</span>
							<div class="reg-int">
								<input type="number" placeholder="请输入LDL-C值" v-model="valueTxt">
								<i>mmol/L</i>
							</div>
						</label>
					</li>
	...
	export default {
		...
		methods:{
			...
			initAjax (data) {
				...
				if( this.valueTxt < 0.01 || this.valueTxt > 50 ){
					this.isLading = false;
					this.valueTxt = '';
					this.show = true;
					this.message = '数值范围应在0.01~50.00mmol/L之间。';
					this.clearStorage();
				}else if( this.valueTxt%1 !== 0 ){
					if(this.valueTxt.toString().split(".")[1].length > 2){
						this.isLading = false;
						this.show = true;
						this.message = '数值范围小数点最多保留2位数';
						this.clearStorage();
					}else{
						...
						.then(response => {
							...
							this.message = '数值范围应在0.01~20.00mmol/L之间。';
						})
					}	
				}else {
					this.$http.post('/webserviceProxy/webservice/wtWebApiH/GetServerData',{
