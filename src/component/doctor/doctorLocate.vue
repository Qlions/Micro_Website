<template>
	...
	<!--<div class="header_content">	
		<div class="head_null" style="width:100%;height:2rem;" ref="head_null"></div>
		<div class="head_content_box">
			<div class="content_51_div" v-if="isHead">
		...
		<header :style="{top:headerMargTop}" v-on:child-say="listenToMyBoy">
			<b @click="backToIndex"><img src="../../assets/image/head-back.png" alt="返回图标"></b>
			<h2>{{titleName}}</h2>
		</header>
		<Loading v-if="isLoading" class="loading"></Loading>
		<div class="addr-search-wrap" :style="{top:startSearchBtn}">
		...
		<div class="addr-search-record" :style="{top:searchLists}">
		...
		<div class="adddr-search-list" v-if="isList" :style="{top:searchListTop}">
			<ul>
...
//					}
			...
			searchInfo(){
				...
				vm.$router.push({path:"/doctor",query:data})
			},
			deleteTxt(){
				let vm = this;
				vm.needReceipt = "";
		...
		mounted(){
			if(sessionStorage.getItem('key_head') == 1){
				//没有头部下载
				this.headerMargTop = 0+"rem";//顶部header				
				this.startSearchBtn = 0.84+'rem';//搜索按钮
				this.searchLists = 3.26+"rem";//搜索历史列表
				this.searchListTop = 4.26+"rem";//
				...
				this.searchListTop = 4.26+"rem";//
				//有头部下载
//				this.headerMargTop = 1.16+"rem";//顶部header
//				this.startSearchBtn = 2+'rem';//搜索按钮
//				this.searchLists = 4.6+"rem";//搜索历史列表
//				this.searchListTop = 4.86+"rem";//搜索列表
			}
			this.init()
		},
		updated () {
...
//				this.startSearchBtn = 2+'rem';//搜索按钮
//				this.searchLists = 4.6+"rem";//搜索历史列表
//				this.$refs.head_null.style.height = 2+'rem';
			}
		},
		watch:{
